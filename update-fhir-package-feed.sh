...
xml ed --inplace -u "//channel/lastBuildDate" -v "$pubDate" $1
xml ed --inplace -u "//channel/pubDate" -v "$pubDate" $1
...
patchedXml=$(xml ed --subnode "//channel" --type "elem" -n "item" -v "$item" $1 | xml unesc | xmllint --format - | xml fo -o)
echo -e "$patchedXml" > $1
rm -rf package/